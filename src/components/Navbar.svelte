...
<header>
  ...
  <div class="relative bg-white border-b border-purple-100 overflow-hidden">
    {#if $identity.university_logo_url}
      <img
        src={$identity.university_logo_url}
        alt=""
        aria-hidden="true"
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-20 sm:h-24 gap-4">
        ...
        <div class="hidden lg:flex flex-col text-xs space-y-1 min-w-0 flex-shrink-0">
          ...
          {/if}
          {#if phone}
            <a href={contactHref(phone)} class="text-gray-600 hover:text-purple-700 transition-colors">
              {phone.value}
            </a>
          {/if}
        </div>
        ...
        <!-- Center: logo + title -->
        <a href="/home" on:click={(e) => go(e, '/home')}
          ...
          <div class="relative flex-shrink-0">
            ...
            <div class="relative w-14 h-14 sm:w-16 sm:h-16 rounded-2xl border-2 border-purple-200 bg-white p-1.5 shadow-[0_0_0_4px_rgba(139,92,246,0.06)] group-hover:border-purple-500 group-hover:shadow-[0_0_0_6px_rgba(139,92,246,0.15)] transition-all duration-300">
              {#if $identity.site_logo_url}
                <img src={$identity.site_logo_url} alt={$identity.site_title} class="w-full h-full object-contain" />
              {/if}
            </div>
          </div>
          <div class="flex flex-col min-w-0">
            <span class="text-lg sm:text-2xl font-bold text-gray-900 leading-tight group-hover:text-purple-800 transition-colors whitespace-nowrap">
              {$identity.site_title || 'Cyber Security'}
            </span>
        ...
        <div class="hidden lg:block flex-shrink-0">
          <a href={mainSiteUrl} target="_blank" rel="noopener noreferrer"
            ...
            <span class="relative">Main Site</span>
          </a>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-14">
        ...
        <div class="hidden md:flex items-center gap-0.5">
          {#each $navItems as item, i (item.id ?? item.href ?? i)}
            {#if item.children && item.children.length}
              <div class="relative group">
                <button
                  on:click={() => toggleDropdown(item.id ?? `dd-${i}`)}
                  on:blur={() => setTimeout(() => { openDropdown = null; }, 150)}
                ...
                >
                  ...
                  <svg class="w-3.5 h-3.5 relative z-10 transition-transform duration-200 {openDropdown === (item.id ?? `dd-${i}`) ? 'rotate-180' : ''}"
                ...
                {#if openDropdown === (item.id ?? `dd-${i}`)}
                  <div class="absolute top-full left-0 mt-1 w-64 rounded-lg overflow-hidden bg-[#3d2877] border border-purple-500/40 shadow-xl shadow-purple-900/40 backdrop-blur-sm">
                    <div class="h-px bg-gradient-to-r from-transparent via-purple-300/60 to-transparent"></div>
                    {#each item.children as child}
                      <a href={child.href} on:click={(e) => go(e, child.href)}
            ...
            {:else if item.external}
              {@const url = externalHref(item)}
              <button
                on:click={(e) => goExternal(e, url)}
              ...
              >
                <span class="relative z-10">{item.text}</span>
                <svg class="w-3 h-3 relative z-10 opacity-70 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {:else}
              <a href={item.href} on:click={(e) => go(e, item.href)}
        ...
        <div class="md:hidden flex items-center gap-2">
          ...
          <span class="text-white text-sm font-medium tracking-wide truncate max-w-[220px]">
            {$identity.site_title || 'Cyber Security'}
          </span>
        </div>

        <button class="md:hidden inline-flex items-center justify-center p-2 rounded-lg text-white hover:bg-white/10 transition-colors"
                on:click={() => (mobileMenuOpen = !mobileMenuOpen)} aria-label="Toggle menu">
        ...
        <div class="relative px-3 py-3 space-y-1">
          {#each $navItems as item, i (item.id ?? item.href ?? i)}
            {#if item.children && item.children.length}
              <div>
                <button
                  on:click={() => toggleDropdown(item.id ?? `m-${i}`)}
                ...
                >
                  <span>{item.text}</span>
                  <svg class="w-4 h-4 transition-transform {openDropdown === (item.id ?? `m-${i}`) ? 'rotate-180' : ''}"
                       fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {#if openDropdown === (item.id ?? `m-${i}`)}
