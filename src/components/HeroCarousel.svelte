...
{#if slides.length === 0}
  <section class="relative w-full h-[60vh] md:h-[80vh] bg-gradient-to-br from-purple-800 via-[#4B338C] to-purple-900 flex items-center justify-center text-white">
    <div class="text-center px-4">
      <div class="text-xs uppercase tracking-[0.3em] text-purple-300 mb-4">Department of</div>
      <h1 class="text-4xl md:text-6xl font-bold mb-3">Cyber Security</h1>
      <p class="text-lg md:text-xl text-purple-200 italic">the university of relevance</p>
    </div>
  </section>
{:else}
  <section class="relative w-full h-[60vh] md:h-[80vh] overflow-hidden bg-black">
    <div class="relative w-full h-full">
      {#each slides as slide, i (slide.id ?? i)}
        ...
        >
          {#if slide.image_url}
            <img src={slide.image_url} alt={slide.headline} class="w-full h-full object-cover" loading={i === 0 ? 'eager' : 'lazy'} />
          {:else}
            <div class="w-full h-full bg-gradient-to-br from-purple-800 to-purple-950"></div>
          {/if}
          ...
          <div class="absolute inset-0 flex items-center justify-center">
            <div class="text-center text-white px-6 max-w-3xl">
              <h2 class="text-3xl sm:text-4xl md:text-6xl font-bold mb-4 drop-shadow-lg">{slide.headline}</h2>
              {#if slide.subtext}
                <p class="text-lg sm:text-xl md:text-2xl text-purple-100 mb-8 drop-shadow">{slide.subtext}</p>
              {/if}
              {#if slide.link_url}
                <a
    ...
    {#if slides.length > 1}
      <button
        on:click={prevSlide} aria-label="Previous"
        class="absolute left-3 sm:left-5 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-2.5 sm:p-3 rounded-full backdrop-blur-sm transition-colors"
      >
      ...
      <button
        on:click={nextSlide} aria-label="Next"
        class="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-2.5 sm:p-3 rounded-full backdrop-blur-sm transition-colors"
      >
      ...
      <div class="absolute bottom-5 left-0 right-0 flex justify-center gap-2">
        {#each slides as _, i}
          <button
            ...
            class="h-2 rounded-full transition-all duration-300 {i === currentSlide ? 'w-8 bg-white' : 'w-2 bg-white/50 hover:bg-white/80'}"
          ></button>
        {/each}
      </div>
    {/if}
