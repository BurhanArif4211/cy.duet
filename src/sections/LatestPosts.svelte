<script>
  ...
  const go = (e, slug) => { e.preventDefault(); router.navigate(`/news/${slug}`); };
</script>

<section class="py-16 px-4 md:px-8 lg:px-16 bg-purple-50/50">
  <div class="max-w-7xl mx-auto">
    <div class="text-center mb-10">
      <div class="text-xs uppercase tracking-[0.3em] text-purple-600 mb-3">Newsroom</div>
      <h2 class="text-3xl md:text-4xl font-bold text-gray-900">Latest News & Updates</h2>
    </div>
    ...
    {:else}
        ...
        {#each posts as p (p.id)}
          ...
          >
            {#if p.cover_image_url}
              <div class="hidden md:block w-full h-48 overflow-hidden bg-purple-100">
            ...
            <div class="p-6 flex-1 flex flex-col">
              ...
              {/if}
              <span class="mt-4 text-purple-600 text-sm font-medium inline-flex items-center gap-1 group-hover:gap-2 transition-all">
                Read more
      ...
      <div class="text-center mt-10">
        <a
          href="/news"
          on:click={(e) => { e.preventDefault(); router.navigate('/news'); }}
